...
def write_batches(groups: dict[str, list[dict]], execution_id: str) -> list[dict]:
    ...
    for category, products in groups.items():
        for i in range(0, len(products), BATCH_SIZE):
            ...
            _s3.put_object(Bucket=_bucket, Key=key, Body=json.dumps(batch), ContentType="application/json")
...
def handler(event, context):
    ...
    print(f"[MergeResults] {len(all_products)} total → {len(unique)} unique → {len(batch_refs)} batches ({len(groups)} categories)")
    return batch_refs
