...
export const handler = async (event: { results?: Array<{ unmatched?: ScrapedProduct[] }>; unmatched?: ScrapedProduct[] }) => {
    const unmatched: ScrapedProduct[] = event.results ? event.results.flatMap((r) => r.unmatched ?? []) : (event.unmatched ?? [])
