...
function extractPackaging(specs: Array<{ key: string; value: string[] }>, name: string, category: string | undefined): string | undefined {
...
export class JumboSpider extends BaseSpider {
    ...
    async _getProductsFromPage(page: unknown, categoryUrl: string): Promise<ScrapedProduct[]> {
        ...
        const slugs = (data.products ?? []).map((p) => p.slug).filter((s): s is string => !!s)
    ...
    _formatProduct(raw: unknown, ...args: unknown[]): ScrapedProduct {
        ...
        const category = categoryNames.length > 1 ? categoryNames[1] : categoryNames.length > 0 ? categoryNames[0] : undefined
